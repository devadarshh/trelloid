...
export default clerkMiddleware(async (auth, req) => {
  ...
  // 1. Not logged in and on a private route → redirect to sign-in
  ...
  // 2. Logged in, no org, and not already on /select-org → redirect to /select-org
  if (userId && !orgId && !isOnSelectOrgPage) {
    const orgSelectionUrl = new URL("/select-org", req.url);
    return NextResponse.redirect(orgSelectionUrl);
  }

  // 3. Logged in and on public route → redirect to org or /select-org
  if (userId && isOnPublicRoute) {
